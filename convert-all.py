...
def create_default_ticket_types(model, new_db_path, default_club_id):
    """Create default ticket types if tickets_tickettype is empty."""
    default_types = [
        {
            'name': 'session',
            'club_id': default_club_id,
            'price': 50.0,
            'description': 'Single session ticket'
        },
    ]
    try:
        existing_types = model.objects.all()
        if not existing_types.exists():
            print("إنشاء أنواع تذاكر افتراضية...")
            for ticket_type in default_types:
                if not check_foreign_key('core_club', 'id', ticket_type['club_id'], new_db_path):
                    print(f"خطأ: club_id={ticket_type['club_id']} غير موجود في core_club")
                    continue
                model.objects.create(**ticket_type)
            print("تم إنشاء أنواع تذاكر افتراضية بنجاح")
    except Exception as e:
        print(f"خطأ في إنشاء أنواع تذاكر افتراضية: {e}")

def get_valid_club_id(new_db_path):
    """Get a valid club_id from core_club."""
    try:
        conn = sqlite3.connect(new_db_path)
        cursor = conn.cursor()
        ...
        print(f"خطأ في جلب club_id من core_club: {e}")
        return None

def migrate_table(model, table_name, old_db_path, new_db_path, migration_summary, default_values=None, default_club_id=None):
    ...
    if table_name == 'tickets_ticket' and default_club_id:
        create_default_ticket_types(models['tickets_tickettype'], new_db_path, default_club_id)
    ...
    for row_index, row in enumerate(rows):
        ...
        if table_name in [
            ...
            if club_id and not check_foreign_key('core_club', 'id', club_id, new_db_path):
                # Try mapping invalid club_id to default_club_id
                if default_club_id:
                    row_data['club_id'] = default_club_id
                else:
        ...
        if table_name == 'tickets_ticket':
            ...
            if ticket_type_name:
                try:
                    ticket_type = models['tickets_tickettype'].objects.get(
                        name=ticket_type_name.lower(),
                        club_id=row_data.get('club_id', default_club_id)
                    )
                    row_data['ticket_type_id'] = ticket_type.id
            ...
            if issued_by_id and not check_foreign_key('accounts_user', 'id', issued_by_id, new_db_path):
                row_data['issued_by_id'] = None

            if 'buyer_name' in row_data:
                row_data['notes'] = row_data['buyer_name']
            if 'issue_date' in row_data:
                try:
                    issue_date = datetime.strptime(row_data['issue_date'], '%Y-%m-%d')
                    row_data['issue_datetime'] = timezone.make_aware(issue_date)
                except ValueError:
                    row_data['issue_datetime'] = timezone.now()
            issue_date_str = row_data.get('issue_date', timezone.now().date()).replace('-', '') if isinstance(row_data.get('issue_date'), str) else row_data.get('issue_date').strftime('%Y%m%d')
            row_data['serial_number'] = f"{issue_date_str}-{str(row_index % 1000 + 1).zfill(4)}"
...
def migrate_all_tables(old_db_path, new_db_path):
    """Migrate data for all required tables, excluding devices, subscriptions_freezerequest, and subscriptions_coachprofile."""
    ...
    # Migrate core_club first and verify
    try:
        migrate_table(models['core_club'], 'core_club', old_db_path, new_db_path, migration_summary)
        if migration_summary['core_club']['success'] == 0 and migration_summary['core_club']['total'] > 0:
            print("خطأ: فشل تهجير core_club. التوقف.")
            print_summary(migration_summary)
            return
    except Exception as e:
        print(f"خطأ في تهجير core_club: {e}")
        print_summary(migration_summary)
        return

    # Get a valid club_id for default ticket types
    default_club_id = get_valid_club_id(new_db_path)
    if not default_club_id:
        print("خطأ: لا يوجد club_id صالح في core_club بعد التهجير.")
        print_summary(migration_summary)
        return
    ...
    tickettype_defaults = {}
    ticket_defaults = {
        'tickets.Ticket.notes': lambda x: x.get('buyer_name', 'Migrated ticket'),
        'tickets.Ticket.serial_number': lambda x: f"{x.get('issue_date', timezone.now().date()).replace('-', '') if isinstance(x.get('issue_date'), str) else x.get('issue_date').strftime('%Y%m%d')}-{str(x.get('row_index', 0) % 1000 + 1).zfill(4)}",
        'tickets.Ticket.issued_by': None,
        'tickets.Ticket.issue_datetime': lambda x: timezone.now(),
    }
    ...
    migrate_table(models['tickets_ticket'], 'tickets_ticket', old_db_path, new_db_path, migration_summary, ticket_defaults, default_club_id=default_club_id)
...
if __name__ == "__main__":
    ...
    import django
    django.setup()
    migrate_all_tables(OLD_DB_PATH, NEW_DB_PATH)