...
const MemberAttendanceChart = ({ memberId }) => {
...
//         console.log('Received attendance data:', result);
    ...
    if (memberId) {
      ...
      setError('معرف العضو غير متوفر');
      setLoading(false);
    }
  }, [memberId]);

  // معالجة البيانات لإنشاء شبكة سنوية (53 أسبوع)
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-full py-12" dir="rtl">
  ...
  if (!data || data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full py-12 space-y-4" dir="rtl">
  ...
  return (
    <div className="relative w-full flex flex-col font-sans" dir="rtl">
      {/* Header Stats */}
