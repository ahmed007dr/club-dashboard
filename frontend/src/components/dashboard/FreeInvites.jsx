...
const InviteList = () => {
  ...
  // Extract unique clubs from invites
  const uniqueClubs = Array.from(
    new Map(
      invites.map(invite => [invite.club_details?.id, invite.club_details])
    ).values()
  ).filter(club => club && club.id && club.name); // Ensure valid clubs
...
console.log('Unique Clubs:', uniqueClubs);
  ...
  const validateForm = () => {
    const errors = {};
    if (!formData.club) errors.club = 'Club selection is required';
  ...
  const handleEditInvite = async (e) => {
    ...
    const cleanedData = {
      ...formData,
      invited_by: formData.invited_by?.toString().trim() === "" ? null : formData.invited_by,
    ...
    try {
      ...
      console.error('Failed to edit invite:', error.response?.data || error.message);
    }
  };
