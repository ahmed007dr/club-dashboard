...
import { fetchMemberSubscriptions, addAttendance } from '../../redux/slices/subscriptionsSlice';
import {
  FaUser,
  FaCalendarAlt,
  FaMoneyBillAlt,
  ...
  useEffect(() => {
    if (memberId) {
      dispatch(fetchMemberSubscriptions(memberId))
        .then((response) => {
          // Only add attendance if subscriptions were successfully loaded
          if (response.payload && response.payload.length > 0) {
            const attendanceData = {
              subscription: response.payload[0].id, // Use first subscription
              attendance_date: new Date().toISOString().split('T')[0] // YYYY-MM-DD format
            };
            dispatch(addAttendance(attendanceData));
          }
        });
    }
  }, [dispatch, memberId]); // Dependencies: dispatch, memberId, subscriptionStatus

  // Function to add attendance for the member
  const addAttendanceForMember = () => {
    const memberIdFromState = memberId;  // Use memberId from params or state

    if (memberSubscriptions.length > 0) {
      const subIndex = memberSubscriptions.findIndex(
        (sub) => sub.member === memberIdFromState
      );

      if (subIndex !== -1) {
        // Example attendance data (adjust as needed)
        const attendanceData = {
          subscription: memberSubscriptions[subIndex].id,  // Subscription ID from the found member
          attendance_date: new Date().toLocaleDateString('en-GB')  // Today's date
        };

        // Dispatch addAttendance action to add attendance to the member's record
        dispatch(addAttendance(attendanceData));
      } else {
        console.log("Member not found in subscriptions");
      }
    }
  };

  if (status === 'loading') return <div className="text-center py-8">
          ...
          {memberSubscriptions.map((sub) => (
            <li key={sub.id} className="p-6 border border-gray-200 rounded-xl shadow-md bg-gray-50 hover:bg-gray-100 transition">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-gray-700 text-[15px] mt-4">
                ...
                <p className="flex items-center gap-2"><FaCalendarAlt /> <strong>تاريخ البداية:</strong> {new Date(sub.start_date).toLocaleDateString('ar-EG')}</p>
                <p className="flex items-center gap-2"><FaCalendarAlt /> <strong>تاريخ النهاية:</strong> {new Date(sub.end_date).toLocaleDateString('ar-EG')}</p>
                <p className="flex items-center gap-2"><FaClock /> <strong>المدة:</strong> {sub.type_details?.duration_days} يوم</p>
                <p className="flex items-center gap-2"><FaMoneyBillAlt /> <strong>السعر:</strong> {sub.type_details?.price?.toLocaleString()} ج.م</p>
                <p className="flex items-center gap-2"><FaCheck /> <strong>المدفوع:</strong> {sub.paid_amount?.toLocaleString()} ج.م</p>
                <p className="flex items-center gap-2"><FaExclamation /> <strong>المتبقي:</strong> {sub.remaining_amount?.toLocaleString()} ج.م</p>
                <p className="flex items-center gap-2">
                  <FaCalendarCheck /> 
                  <strong>أيام الحضور:</strong> 
