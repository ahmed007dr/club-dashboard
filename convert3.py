...
def migrate_table(model, table_name, old_db_path, new_db_path, migration_summary, default_values=None):
    ...
    for row_index, row in enumerate(rows):
        ...
        if table_name == 'subscriptions_subscription':
            ...
            if coach_id and not check_foreign_key('employees_employee', 'id', coach_id, new_db_path):
        ...
        if table_name == 'subscriptions_coachprofile':
            ...
            if user_id and not check_foreign_key('employees_employee', 'id', user_id, new_db_path):
        ...
        if table_name == 'attendance_attendance':
            ...
            if approved_by_id and not check_foreign_key('employees_employee', 'id', approved_by_id, new_db_path):
        ...
        if table_name == 'staff_shift':
            ...
            if staff_id and not check_foreign_key('employees_employee', 'id', staff_id, new_db_path):
        ...
        if table_name == 'staff_staffattendance':
            ...
            created_by_id = row_data.get('created_by_id')
            if created_by_id and not check_foreign_key('employees_employee', 'id', created_by_id, new_db_path):
                errors.append(f"Skipping record in {table_name}: invalid created_by_id: {created_by_id}, data: {row_data}")
                migration_summary[table_name]['failed'] += 1
                continue
        ...
        if table_name == 'finance_expense':
            ...
            if paid_by_id and not check_foreign_key('employees_employee', 'id', paid_by_id, new_db_path):
            ...
            if related_employee_id and not check_foreign_key('employees_employee', 'id', related_employee_id, new_db_path):
        ...
        if table_name == 'finance_income':
            ...
            if received_by_id and not check_foreign_key('employees_employee', 'id', received_by_id, new_db_path):
        ...
        if table_name == 'tickets_ticket':
            ...
            if issued_by_id and not check_foreign_key('employees_employee', 'id', issued_by_id, new_db_path):
...
def _map_role_to_job_title(role):
    """تحويل الدور (role) إلى عنوان وظيفي (job_title)"""
    role_mapping = {
        'owner': 'Owner',
        'admin': 'Admin',
