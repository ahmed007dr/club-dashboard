...
def get_object_from_id_or_name(model, value, fields=['id']):
    """
    Retrieve an object by ID only. Returns None if not found.
    """
    try:
        return model.objects.get(id=int(value.strip()))
