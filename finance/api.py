...
def parse_date_with_optional_time(date_str):
    try:
        ...
        try:
            ...
            dt = datetime.strptime(date_str, '%d-%m-%Y').replace(hour=0, minute=0, second=0)
    return timezone.make_aware(dt)

def apply_common_filters(queryset, request, user_field=None, source_category_field=None, restrict_to_attendance=False, shift_id=None):
    ...
    if shift_id:
        try:
            attendance = StaffAttendance.objects.get(id=shift_id, club=request.user.club)
            end_time = attendance.check_out if attendance.check_out else timezone.now()
            queryset = queryset.filter(date__gte=attendance.check_in, date__lte=end_time)
            applied_filters.append(f"shift_id={shift_id}")
        except StaffAttendance.DoesNotExist:
            logger.error(f"Shift ID {shift_id} not found for club {request.user.club}")
            queryset = queryset.none()
            applied_filters.append("shift_id=None (not found)")
    elif restrict_to_attendance and request.user.role not in FULL_ACCESS_ROLES:
        attendances = StaffAttendance.objects.filter(
            staff=request.user, club=request.user.club
        ).select_related('staff', 'club')
        if not attendances.exists():
            logger.debug("No attendance records found for user", extra={'force': True})
            queryset = queryset.none()
            applied_filters.append("no_attendance")
        else:
            latest_attendance = attendances.order_by('-check_in').first()
            end_time = latest_attendance.check_out if latest_attendance.check_out else timezone.now()
            queryset = queryset.filter(date__gte=latest_attendance.check_in, date__lte=end_time)
            applied_filters.append(f"attendance={latest_attendance.id}")

    if not shift_id and not restrict_to_attendance:
    ...
    if query_params.get('user'):
        ...
        if user_obj and user_obj.club == request.user.club:
            if request.user.role in ['owner', 'admin'] or user_obj == request.user:
                ...
                queryset = queryset.none()
                applied_filters.append(f"user=None (access denied)")
        else:
            queryset = queryset.none()
        ...
        try:
            ...
            if request.user.role not in ['admin', 'owner']:
                ...
                expenses = apply_common_filters(
                    ...
                    restrict_to_attendance=True, shift_id=shift_id
                )
            else:
                expenses = apply_common_filters(
                    expenses, request, user_field='paid_by', source_category_field='category'
...
def income_detail_api(request, pk):
    ...
    if request.method == 'GET':
        ...
        translated_data = serializer.data
        if translated_data['source']:
            source_name = IncomeSource.objects.get(id=translated_data['source']).name
            translated_data['source'] = SOURCE_TRANSLATIONS.get(source_name, source_name)
...
def income_summary(request):
    ...
    try:
        ...
        if not any(request.query_params.get(param) for param in ['date', 'start_date', 'end_date', 'source', 'user', 'amount', 'description', 'shift_id']):
            return Response({'error': 'يجب تحديد معايير البحث (تاريخ، مدة زمنية، مصدر، مستخدم، مبلغ، وصف، أو shift_id).'}, status=status.HTTP_400_BAD_REQUEST)
      
        # Apply common filters
        incomes = apply_common_filters(incomes, request, user_field='received_by', source_category_field='source', restrict_to_attendance=request.user.role not in FULL_ACCESS_ROLES, shift_id=request.query_params.get('shift_id'))
        ...
        if request.query_params.get('details', 'false').lower() == 'true':
            ...
            for income in translated_details:
                if income['source']:
                    source_name = IncomeSource.objects.get(id=income['source']).name
                    income['source'] = SOURCE_TRANSLATIONS.get(source_name, source_name)
                income['payment_method'] = PAYMENT_METHOD_TRANSLATIONS.get(
        ...
        logger.error(f"Unexpected error in income_summary: {str(e)}", extra={'force': True})
...
def expense_summary(request):
    """Generate expense summary with required filters."""
    if not request.user.club:
        logger.error(f"User {request.user.username} has no associated club")
        return Response({'error': 'غير مسموح: المستخدم ليس مرتبط بنادي.'}, status=status.HTTP_403_FORBIDDEN)
    ...
    try:
        ...
        # Require at least one filter to proceed
        if not any(request.query_params.get(param) for param in ['date', 'start_date', 'end_date', 'category', 'user', 'related_employee', 'amount', 'description', 'shift_id']):
            return Response({'error': 'يجب تحديد معايير البحث (تاريخ، مدة زمنية، فئة، مستخدم، موظف مرتبط، مبلغ، وصف، أو shift_id).'}, status=status.HTTP_400_BAD_REQUEST)
       
        # Apply common filters
        expenses = apply_common_filters(expenses, request, user_field='paid_by', source_category_field='category', restrict_to_attendance=request.user.role not in FULL_ACCESS_ROLES, shift_id=request.query_params.get('shift_id'))
        ...
        logger.error(f"Error in expense_summary: {str(e)}", extra={'force': True})
        return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
    except Exception as e:
        logger.error(f"Unexpected error in expense_summary: {str(e)}", extra={'force': True})
        return Response({'error': 'حدث خطأ غير متوقع'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def finance_overview(request):
    """Generate financial overview with required filters."""
    if not request.user.club:
        logger.error(f"User {request.user.username} has no associated club")
        return Response({'error': 'غير مسموح: المستخدم ليس مرتبط بنادي.'}, status=status.HTTP_403_FORBIDDEN)
...
def expense_all_api(request):
    """List all expenses with required filters."""
    if not request.user.club:
        logger.error(f"User {request.user.username} has no associated club")
        return Response({'error': 'غير مسموح: المستخدم ليس مرتبط بنادي.'}, status=status.HTTP_403_FORBIDDEN)
...
def income_all_api(request):
    """List all incomes with required filters."""
    if not request.user.club:
        logger.error(f"User {request.user.username} has no associated club")
        return Response({'error': 'غير مسموح: المستخدم ليس مرتبط بنادي.'}, status=status.HTTP_403_FORBIDDEN)
        ...
        if date_str:
            try:
                ...
                try:
                    ...
                    date_filter = {'date__date': filter_datetime.date()}
                except ValueError:
                    # Fallback to date only, filter for the whole day
                    filter_date = datetime.strptime(date_str, '%Y-%m-%d').date()
                    date_filter = {'date__date': filter_date}
            except ValueError:
...
def employee_daily_report_api(request):
    ...
    try:
        ...
        # For owner/admin roles: support custom dates without restrictions
        if request.user.role in FULL_ACCESS_ROLES:
            if start_date and end_date:
                ...
                try:
                    start_date = parse_date_with_optional_time(start_date)
                    end_date = parse_date_with_optional_time(end_date)
                    logger.debug(f"Parsed datetime: start_date={start_date}, end_date={end_date}")
                except ValueError:
        ...
        # For other roles: use attendance period
        else:
            shift_id = request.query_params.get('shift_id')
            if shift_id:
                try:
                    attendance = StaffAttendance.objects.get(id=shift_id, club=request.user.club, staff=employee)
                    start_date = attendance.check_in
                    end_date = attendance.check_out if attendance.check_out else today
                except StaffAttendance.DoesNotExist:
                    return Response({'error': 'معرف الشيفت غير موجود.'}, status=status.HTTP_404_NOT_FOUND)
            else:
                ...
                # If custom dates are provided, ensure they are within the attendance period
                if start_date and end_date:
                    start_date = start_date.split('+')[0].split('%2B')[0]
                    end_date = end_date.split('+')[0].split('%2B')[0]
                    ...
                    try:
                        custom_start = parse_date_with_optional_time(start_date)
                        custom_end = parse_date_with_optional_time(end_date)
                        logger.debug(f"Parsed custom datetime: custom_start={custom_start}, custom_end={custom_end}")
                    except ValueError:
                        return Response({'error': 'صيغة التاريخ غير صحيحة (متوقع: YYYY-MM-DD أو YYYY-MM-DDTHH:MM:SS)'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        logger.error(f"Unexpected error in employee_daily_report_api: {str(e)}", exc_info=True)
        return Response({'error': 'حدث خطأ غير متوقع'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

@api_view(['GET'])
@permission_classes([IsAuthenticated])
def generate_daily_report_pdf(request):
    """Generate a daily report PDF with required filters."""
    if not request.user.club:
        logger.error(f"User {request.user.username} has no associated club")
        return Response({'error': 'غير مسموح: المستخدم ليس مرتبط بنادي.'}, status=status.HTTP_403_FORBIDDEN)
    ...
    try:
        ...
        report_data = {
            ...
            'incomes': [
                {'date': income.date.strftime('%Y-%m-%d %H:%M:%S'), 'source': SOURCE_TRANSLATIONS.get(income.source.name, income.source.name), 'amount': f"{income.amount:.2f}"}
